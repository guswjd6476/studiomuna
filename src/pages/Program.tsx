...
export async function getStaticProps() {
  const { data: programs, error } = await supabaseClient.from('programs').select('*');
...
const Program = ({ programs }: { programs: any[] }) => {
  ...
  const getPublicImageUrl = (filename: string) => {
    return `https://lyoiltescwuhcqomxzbg.supabase.co/storage/v1/object/public/program-images/${filename}`;
  };
    ...
    <div className="bg-[#fdfcd7] min-h-screen px-8 md:px-20 py-16 font-sans text-[#296129]">
      ...
      {["1", "2", "3", "4"].map((type) => (
        ...
        >
          ...
          {programs.filter((program) => program.type === type).map((program, index) => {
  // 여기서 콘솔 찍기
  console.log('이미지 URL:', getPublicImageUrl(program.image_filename));  // ← 이미지 URL을 찍어보기
  ...
  return (
    ...
    >
      <div className="w-[300px] h-[300px] overflow-hidden rounded-xl shadow-lg mx-auto flex justify-center items-center">
        <Image
          src={getPublicImageUrl(program.image_filename)} // 이미지 URL 받아오기
          alt={program.title}
          width={300}
          height={300}
            ...
            <div className="flex flex-col md:flex-row gap-8">
              <div className="w-full md:w-1/2 flex justify-center items-center">
                <div className="w-[320px] h-[320px] overflow-hidden rounded-xl shadow-lg transition-transform transform hover:scale-105">
                  <Image
                    src={getPublicImageUrl(selectedProgram.image_filename)}
