...
export async function getStaticProps() {
  ...
  const eventsWithUrls = (recentEvents || []).map((event) => ({
    ...event,
    publicUrl: `https://lyoiltescwuhcqomxzbg.supabase.co/storage/v1/object/public/event-images/${event.image_url}`,
  }));

  return {
    props: {
      recentEvents: eventsWithUrls,
    },
    revalidate: 10,
  };
}
    ...
    <div className="min-h-screen bg-[#fdfcd7] flex flex-col items-center">
      ...
      <section className="w-[90%] max-w-4xl mt-[100px] my-16 p-6 flex flex-col lg:flex-row justify-between items-start gap-12 font-gowun">
        <div className="w-full lg:w-1/2 max-w-[600px]">
          <h2 className="text-4xl font-bold text-[#296129] mb-8" style={{
            textShadow: "2px 2px 0px white, -2px -2px 0px white, 2px -2px 0px white, -2px 2px 0px white",
          }}>
            TEXT-HIP <br /> - 고전문학 프로그램
          </h2>
          <p className="text-lg text-gray-700 leading-relaxed">
            <strong>TEXT_HIP</strong>은 고전문학을 현대적으로 재해석하며, <br />
            삶에 연결되는 통찰을 나누는 프로그램입니다. <br /><br />
            스튜디오 무나에서는 고전 속 인물과 주제들을 바탕으로, <strong>&apos;summary 북토킹&apos;</strong>을 진행하고 있어요.
            <br />책을 다 읽지 않아도 핵심을 짚어가는 방식으로, 누구나 쉽게 고전과 친해질 수 있도록 도와줍니다. <br /><br />
            또한, 고전의 메시지를 자기 삶에 적용할 수 있도록 설계된 <strong>&apos;고전 기반 자기계발 프로그램&apos;</strong>도 함께 운영
            중입니다. 철학, 문학, 심리학이 어우러진 이 여정을 통해, 우리는 자신만의 목소리와 방향을 찾아갑니다. <br /><br />
      ...
      <section className="w-[90%] max-w-4xl my-10 p-6 mb-[120px] font-gowun">
        <h2 className="text-4xl font-bold text-[#296129] text-left mb-10" style={{
          textShadow: "2px 2px 0px white, -2px -2px 0px white, 2px -2px 0px white, -2px 2px 0px white",
        }}>
          최근 행사
        </h2>
